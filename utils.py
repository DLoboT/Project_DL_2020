...
class ClusteringLayer(Layer):
    """
    The layer calculate the distance between each sample of the embedding space
...
class KmeansLoss(Layer):
    ...
    """
    def __init__(self, n_clusters, weights=None, L_prop = False, **kwargs):
        if 'input_shape' not in kwargs and 'input_dim' in kwargs:
            kwargs['input_shape'] = (kwargs.pop('input_dim'),)
    ...
    def build(self, input_shape):
        ...
        if self.L_prop:
            self.y_prob_mb = self.add_weight(shape=(self.n_clusters,), initializer='glorot_uniform', name='y_prob_mb')
        if self.initial_weights is not None:
            self.set_weights(self.initial_weights)
            del self.initial_weights
        self.built = True
    ...
    def call(self, dists, **kwargs):
        ...
        if self.L_prop:
#            print(self.alpha.get_shape().as_list())
#            print(self.y_prob_mb.get_shape().as_list())
            stack_softmax = tf.stack(list_softmax)
            stack_softmax = tf.transpose(stack_softmax)
            kmeans_loss = self.y_prob_mb*(tf.log(tf.reduce_mean(stack_softmax, [0]))) 
...
def Loss_KM(lambda_ = 1, L_prop = False):
    """
    """
    def loss(y_true, y_pred):

        if not L_prop:
            km_loss = tf.reduce_mean(y_pred) * lambda_
        else:
            km_loss = (-tf.reduce_sum(y_pred + 1e-7))* lambda_
