...
import logging
import os
from utils import Params, load_image, load_norm, check_folder, cluster_acc
from utils import  split_train_val, target_distribution, split_test, ratio_per_class, KmeansLoss, Loss_KM, Loss_AE
from utils import mapping_labels, check_overwriting_models, write_metrics_k_means, f_func, g_func, ClusteringLayer
...
parser.add_argument('--pretrain', default=False, help="True for pre-train the autoencoder and the cluster representatives")
parser.add_argument('--DKM', default=True, help="Fine Tuning with the DKM method")
parser.add_argument('--L_prop', default=False, help="Include the proportion term in the cost function")
...
#%%    
    ...
    if args.annealing and not args.pretrain:
    ...
    elif not args.annealing and args.pretrain:
       constant_value = 1  # specs.embedding_size # Used to modify the range of the alpha scheme
       max_n = 40  # Number of alpha values to consider (constant values are used here)
    ...
    for i in range(1):
        ...
        if args.pretrain:
            ...
            AE_generator = DataGenerator(data_all, labels_all, coords, index_test, params.channels, params.patch_size, 
    ...
    if args.DKM:
        ...
        kmeans_loss = KmeansLoss(n_clusters, L_prop = args.L_prop, trainable=False, name='kmeans')(clustering_layer)     

        Deep_km = Model(inputs=encoder.input, outputs=[autoencoder.output, kmeans_loss]) 
        Deep_km.summary()
 
        Deep_km.compile(optimizer=SGD(lr=1e-4), loss=[Loss_AE(), Loss_KM(args.lambda_, args.L_prop)])
        
        if args.pretrain:
            Deep_km.get_layer(name='clustering').set_weights([kmeans.cluster_centers_])
        
        index, coords = split_test(coords_sq1, coords_sq2)      
        #        coords, index_tr, index_val = split_train_val(coords_sq1, coords_sq2 )
