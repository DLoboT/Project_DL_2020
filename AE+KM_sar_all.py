...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  5 16:59:10 2020
...
from models import ClusteringLayer, autoencoderConv2D_1, Metrics, cnn, autoencoderConv2D_2
...
parser.add_argument('--samples_per_epoch', default=300000, help="Samples per class")

parser.add_argument('--train', default=True, help="True for pre-train the autoencoder")
parser.add_argument('--DEC', default=False, help="DEC method")

# DEC parameters
parser.add_argument('--save_dir', default='./results/dec')
parser.add_argument('--maxiter', default=2e3, type=int)
...
if __name__ == '__main__':
    ...
    assert os.path.isdir(args.img_sar), "Couldn't find the dataset at {}".format(args.img_opt)
    ...
    # Class 4 as others
    if args.train:
        label_seq1[mask_img==0] = 0
        label_seq2[mask_img==0] = 0
    else:
...
#%%    
    ...
    dims_ae = [32, 64, 128, 256, 128]
    # Define generators            
    dim = (params.patch_size, params.patch_size, params.channels)
    
#    autoencoder, encoder = cnn(input_shape = dim, classes = params.classes)
    autoencoder, _ = autoencoderConv2D_2(input_shape=dim, filters = dims_ae)

    optimizer = Adam(lr=1e-3)
#    autoencoder.compile(loss=[categorical_focal_loss(alpha=[ratio.tolist()], gamma=2.)],metrics=["accuracy"], optimizer=optimizer)
    ...
    for i in range(2,3,1):
        ...
        if args.train:
            ...
            coords_train, index_tr, index_val= split_train_val(coords_sq1, coords_sq2)
#            y_true = labels_all[coords[0][index_tr[:5000,0]], coords[1][index_tr[:5000,0]], index_tr[:5000,1]]
#            print(np.unique(y_true, return_counts = True))

            # Function to check model overwriting            
            check_overwriting_models(model_k, file_output, args.restore_from)
                                      
            training_generator = DataGenerator(data_all, None, coords_train, index_tr, params.channels, 
            ...
            validation_generator = DataGenerator(data_all, None, coords_train, index_val, params.channels, 
                                                 params.patch_size, params.batch_size, dim, params.classes)
...
#%%  Step 1: pretrain the autoencoder             
            ...
            conf_earley =  ['val_loss', 0.001, 0.001, 'min', 10]
            checkpointer = ModelCheckpoint(file_output, monitor=conf_earley[0], mode = conf_earley[3],save_best_only=True)
            
            earlystop = EarlyStoppingByLossVal(monitor = conf_earley[0], min_delta = conf_earley[1], 
                               value = conf_earley[2], mode = conf_earley[3], 
                               patience = conf_earley[4], verbose = 1)    
            
            # Train model on dataset
            history = autoencoder.fit_generator(generator=training_generator,
                                validation_data=validation_generator,
                                epochs = params.num_epochs, callbacks = [checkpointer, earlystop])
            ...
            plt.savefig(os.path.join(args.model_dir,'loss_train_val.png'), dpi = 300, format='png', bbox_inches = 'tight')
            plt.clf()
            plt.close()
...
#%% Build Clustering model 
    ...
    #%%  Step 2: initialize cluster centers using k-means    
        ...
        write_metrics_k_means(args.save_dir, y_true, y_pred, index_test, 'AE+Kmeans')
        
#        save_metrics(model_k, y_true, y_pred, params, 'AE+Kmeans')
#               
#        t_SNE_visualization(args.model_dir, extract_features, y_pred)
#        sys.exit()
#        Visualization_kmeans(q, y_pred)
#        plt.savefig('CF_DEC.png') 
#%% Step 3: deep clustering (DEC)
        ...
        logfile = open(args.save_dir + '/dec_log.csv', 'w')
        ...
        logfile.close()
        print('saving model to: ' + args.save_dir + '/DEC_model_final.h5')
        model.save_weights(args.save_dir + '/DEC_model_final.h5')
        
        acc = cluster_acc(y_true, y_pred)
        # Show the final results
        print('acc_DEC:', acc)
        write_metrics_k_means(args.save_dir, y_true, y_pred, index_test, 'DEC')
